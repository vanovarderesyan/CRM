"use strict";
const { SubcontractorDao: dao } = require("../../dao/subcontractor.dao");
...
class SubcontractorController {
    ...
    updateSubcontractorContracts(req, res) {
        dao.updateSubcontractorContracts(req.body).then((result) => {
            res.status(result.statusCode).json(result.data);
        }).catch((err) => {
            res.status(err.statusCode).json(err.data);
        })
    ...
    getAllProjectBySubcontractorId(req, res) {
        dao.getAllProjectBySubcontractorId(req.params.Id).then((result) => {
            let count = 0;
    ...
    getSubcontractorByQuery(req, res) {
...
module.exports.SubcontractorController = new SubcontractorController();
