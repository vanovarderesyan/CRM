const { FilialDepartmenteQueries: query } = require('./queries/filial-department.queries');
...
class FilialDepartmenteDao {
    signUp(data) {
        return new Promise((resolve, reject) => {
            const conditions = [data.DepartmentId, data.FilialId];
    ...
    getDepartmentByFilialId(Id) {
        return new Promise((resolve, reject) => {
            connectionPool.query(query.getDepartmentByFilialId, [Id], (err, result) => {
                console.log(result)
                if (err) {
                    return reject(RRS.create(418, err));
                }
...
module.exports.FilialDepartmenteDao = new FilialDepartmenteDao();