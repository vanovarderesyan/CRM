const { CompanyFilialeQueries: query } = require('./queries/company-filial.queries');
...
class CompanyFilialeDao {
    signUp(data) {
        return new Promise((resolve, reject) => {
            const conditions = [data.CompanyId, data.FilialId];
    ...
    getFilialByCompanyId(Id) {
        return new Promise((resolve, reject) => {
            connectionPool.query(query.getFilialByCompanyId, [Id], (err, result) => {
                console.log(result)
                if (err) {
                    return reject(RRS.create(418, err));
                }
...
module.exports.CompanyFilialeDao = new CompanyFilialeDao();